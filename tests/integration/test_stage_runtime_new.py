...
def test_stage_runtime_deterministic_snapshot() -> None:
    ...
    runtime_a = StageRuntime(config, stage, registry, seed=derive_stage_seed(config.replay_seed, stage.stage_id))
    runtime_b = StageRuntime(config, stage, registry, seed=derive_stage_seed(config.replay_seed, stage.stage_id))
